...
44. **Upstream CLI/REPL intake from `ultraworkers/claw-code` must be selective for local-Qwen fork** — filed 2026-04-22. Current fork is `214` commits behind `upstream/main`, but most upstream deltas include Anthropic-first provider/auth and broader runtime surfaces we do not want to absorb wholesale. For this fork, the high-value import lane is CLI/REPL behavior and JSON parity only. Candidate intake set should prioritize commits touching `rust/crates/rusty-claude-cli/src/main.rs` and `rust/crates/commands/src/lib.rs` with no provider client changes. Initial shortlist from `HEAD..upstream/main` (to cherry-pick/test in small batches):
- REPL/CLI parser and dispatch hardening:
  - `4cb8fa0` (reject empty/whitespace prompt fallthrough)
  - `f3f6643` (did-you-mean guard for subcommand typos)
  - `a3270db` (reject unrecognized suffix args on diagnostic verbs)
  - `47aa1a5` (surface command name in "not yet implemented" REPL output)
  - `60ec2ae` (wire `/tokens` and `/cache` aliases to implemented `/stats`)
- Resume/JSON parity improvements:
  - `4f670e5`, `cf129c8`, `3168642`, `78dca71`, `7587f2c`, `7ec6860`, `3ed27d5`, `11e2353`
- Operator UX and deterministic diagnostics:
  - `7763ca3` (unified subcommand help contract)
  - `79352a2` (hint `--output-format json` when `--json` is typed)
  - `541c5bb` (actionable `claw state` guidance)
- Scope guard:
  - exclude commits that modify provider routing/auth behavior unless explicitly requested.
- Next action:
  - create a dedicated sync branch and cherry-pick the shortlist in topic batches (`parser`, `resume-json`, `repl-ux`), running `cargo test -p commands -p claw-cli` after each batch.

45. **180s executor timeout is too short for 64k-context, quality-first long tasks** — filed 2026-04-22 and patched in local harness. During traced benchmark run (`/tmp/qwen_matrix_debug_trace_1776845652`), task 1 executor timed out at `180s` (`rc=124`) before delivering a stable edit set, forcing additional passes. With large prompt context and ~30 tok/s throughput assumptions, executor stages need a materially larger wall clock budget. **Fix landed in harness:** add `QWEN35_MATRIX_EXECUTOR_TIMEOUT_SECS` (default `1200`) and apply it specifically to executor/pass/retry stages; planner/repair keep default `QWEN35_MATRIX_TIMEOUT_SECS` (`180`). Keep monitoring for overrun/idle regressions and tune by repo profile.

46. **Post-timeout harness tuning must balance throughput and strict pass quality** — filed 2026-04-22 after reruns. Full rerun with added executor guardrails (`/tmp/qwen_matrix_perf_guard_20260422_141026`) improved first-7-task wall time by `-17.60%` versus the earlier post-timeout run (`1,427,123 ms` vs `1,731,993 ms`), but strict quality fell (`6/7` vs `7/7`) and full-run strict pass ended `6/8`. Follow-up experiment that forced objective-gated passes/retries (canonical late-task slice `/tmp/qwen_matrix_canonical_6_8_20260422_150653`) increased failing-path runtime without recovering the persistent docs-contract failure (`task 3` in slice). **Decision:** keep speed-positive guardrails (executor stall limits + context-window short-circuit) and drop forced objective-gated follow-up. **Next action:** introduce task-class-specific retry guidance for docs/contract tasks (task-8 class) rather than globally forcing extra passes.

47. **Runtime profile split is necessary: no single timeout profile currently gives both prior throughput and strict integrity** — filed 2026-04-22 after additional reruns. New harness profile control (`QWEN35_MATRIX_RUNTIME_PROFILE`) was added to expose `quality` / `balanced` / `fast` defaults without losing env-override control. Measured on identical top-3 slice:
- `balanced` run (`/tmp/qwen_matrix_balanced_top3_20260422_153116`): `3/3` strict pass, `759,752 ms` total (quality preserved, but `+44.50%` slower vs prior top-3 reference `/tmp/qwen_matrix_perf_guard_20260422_141026` at `525,767 ms`).
- `fast` run (`/tmp/qwen_matrix_fast_top3_20260422_154504`): `0/3` strict pass, `188,791 ms` total (`-64.09%` wall time) with repeated zero-edit executor exits.
**Decision:** keep `balanced` as default integrity profile for now, mark `fast` as smoke-only, and add a middle-ground iteration profile that retains edit execution while capping long executor drift.

48. **Balanced-profile long-run drift regression was reduced with executor path/toolset hardening** — filed and rerun on 2026-04-22. Observed regression pattern in prior balanced top-3 run (`/tmp/qwen_matrix_balanced_top3_20260422_153116`) was repeated tool churn on task 2 (`37` executor-family tool calls) and `/app/...` path confusion in executor logs. Harness updates applied in `scripts/run_qwen35_ladder.sh`:
- enforce worktree-root path contract in executor prompts (explicitly disallow `/app/...` absolute paths),
- remove `bash` from first-pass executor toolset (retain `bash` for retry/repair only),
- add objective-gated `QWEN35_MATRIX_EXECUTOR_BASH_LOOP_CUTOFF` follow-up stop,
- emit explicit `bash_calls` in stage metrics/log lines for attribution.
**Benchmark evidence:** rerun on same balanced top-3 slice (`/tmp/qwen_matrix_post_bash_regression_debug`) preserved strict quality (`3/3`) and reduced wall time to `606,924 ms` (`-20.12%` vs `759,752 ms`). Task deltas: task 1 `-25.92%`, task 2 `-34.06%`, task 3 `+3.57%` (task 3 remains the residual drift pocket due extra pass/retry). **Next action:** tighten pass/retry continuation criteria for docs/config task class so task-3-style late follow-up does not overrun without additional strict-quality gain.

49. **Residual executor churn from `no_progress_stalled` was reduced with objective-success short-circuiting** — filed and rerun on 2026-04-22. After #48, task 3 still consumed extra pass/retry because executor ended with `stop_reason=no_progress_stalled` even when objective checks were already satisfiable. Harness update in `scripts/run_qwen35_ladder.sh`: extend stage acceptance logic so `no_progress_stalled` terminal failures are accepted when objective checks pass (same guard already used for context-window failures). **Benchmark evidence:** `/tmp/qwen_matrix_post_stall_accept_20260422_194524` on the same balanced top-3 slice kept strict pass `3/3` and improved wall time to `499,062 ms` (`-34.31%` vs baseline balanced run `/tmp/qwen_matrix_balanced_top3_20260422_153116`, and `-17.77%` vs `/tmp/qwen_matrix_post_bash_regression_debug`). Biggest gains concentrated on prior drift tasks: task 2 `180,100 ms`, task 3 `174,831 ms`. **Next action:** keep this acceptance path and add a lightweight guard to distinguish truly-repeated no-op edit loops from valid late-stage verify/consolidation reads so we do not prematurely accept pathological runs.

50. **Upstream CLI/REPL intake batch 1 (parser + REPL UX) applied with fork-compat adjustment** — executed on 2026-04-22 branch `sync/upstream-cli-repl-batch1-20260422`. Imported targeted upstream commits from shortlist #44: `f3f6643` (typo did-you-mean), `a3270db` (diagnostic suffix rejection), `47aa1a5` (repl not-implemented message includes command name), `60ec2ae` (`/tokens` + `/cache` aliases to `/stats`), plus code-only intake of `4cb8fa0` prompt-fallthrough empty-input rejection. One compatibility patch was required for this fork (`1739a90`): keep shorthand prompt behavior for non-flag multi-word input (`help me debug`) while retaining flag-like misuse rejection from `a3270db`. **Verification:** `cargo test -p commands` passed (`36/36`); `cargo test -p rusty-claude-cli` passed all new parser/repl paths and retained only the pre-existing `mock_parity_harness` generated-file failure. **Next action:** proceed with shortlist batch 2 (`resume/json` commits: `4f670e5`, `cf129c8`, `3168642`, `78dca71`, `7587f2c`, `7ec6860`, `3ed27d5`, `11e2353`) with same scope guard (no provider/auth routing churn).

51. **Upstream CLI/REPL intake batch 2 (resume JSON parity) applied with fork-local resume-cwd preservation** — executed on 2026-04-22 branch `sync/upstream-cli-repl-batch1-20260422`. Imported shortlist #50 resume/json set: `4f670e5`, `cf129c8`, `3168642`, `78dca71`, `7587f2c`, `7ec6860`, `3ed27d5`, `11e2353`. Scope remained CLI/commands only (`rust/crates/rusty-claude-cli/src/main.rs`, `rust/crates/commands/src/lib.rs`), with no provider/auth routing changes. One conflict occurred on `3168642` and was resolved by preserving fork behavior for resumed `/diff` against `session_path.parent()` while still adding JSON parity output. **Verification:** `cargo test -p commands` remained green (`36/36`), `cargo test -p rusty-claude-cli` passed all core suites (`172` main + `7` cli_flags + `2` compact_output) and retained only the known pre-existing `mock_parity_harness` generated-file failure. **Next action:** start upstream batch 3 for operator UX polish (`7763ca3`, `79352a2`, `541c5bb`) under the same provider/auth exclusion rule.
